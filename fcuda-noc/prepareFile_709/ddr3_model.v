...
module ddr3_model (
    ...
    reg [2048:1] mem_write_file;
    reg [2048:1] mem_read_file;
    //reg [2048:1] addr_read_file;
    //reg [2048:1] addr_write_file;
    integer count_write;
    integer count_read;
...
`else
            ...
            if (get_index(addr)) begin
                ...
                $display("WRITE ADDR FAILED: addr=%h, bank=%d, row=%d, col=%d, memory_used=%d, data=%h", addr, bank, row, col, memory_used, data);
                address[memory_used] = addr;
                memory[memory_used] = data;
                memory_used = memory_used + 1;
            end
            $sformat(mem_write_file, "mem_final_%0d.hex", comp_no);
            $writememh(mem_write_file, memory);
            count_write = count_write + 1;
            $display("[%m] WRITE BEGINS: at time = %t addr = %h, bank = %d, row = %d, col = %d, memory_index = %d, memory_used = %d, data = %h, count_write = %d", $time, addr, bank, row, col, memory_index, memory_used, data, count_write);
...
`else
            ...
            if (get_index(addr)) begin
                ...
                $display("READ ADDR FAILED: addr=%h, bank=%d, row=%d, col=%d, memory_used=%d", addr, bank, row, col, memory_used);
                data = {BL_MAX*DQ_BITS{1'bx}};
            end
            count_read = count_read + 1;
            $display("[%m] READ BEGINS: at time = %t addr = %h, bank = %d, row = %d, col = %d, memory_index, memory_used = %d, data = %h count_read = %d", $time, addr, bank, row, col, memory_index, memory_used, data, count_read);
`endif
        end
    endtask

    //TAN
    integer check_init = 0;
    reg [DQ_WIDTH-1:0] mem_data[NUM_COMP*DATA_SIZE-1:0];
    reg [31 : 0] mem_init[2559 : 0];

    always @(*) begin: init_mem_data
      integer i;
      integer j;
    //initial begin: init_mem_data
      if (init_calib_complete && !check_init) begin
        $display("[%m] MEM INIT AT %t", $time);
        $sformat(mem_read_file, "data_init_1024.hex");
        $readmemh(mem_read_file, mem_init);

        for (i = 0; i < 2560 / (DQ_WIDTH / 32); i = i + 1) begin
          for (j = 0; j < DQ_WIDTH / 32; j = j + 1) begin
            $display("[%m] CHECK INIT DATA %d %h, time=%t", i * DQ_WIDTH / 32 + j, mem_init[i * DQ_WIDTH / 32 + j], $time);
            mem_data[i][32 * j +: 32] = mem_init[i * DQ_WIDTH / 32 + j];
          end
        end
        $sformat(mem_read_file, "test_%0d.hex", comp_no);
        $writememh(mem_read_file, mem_data);

        for (i = 0; i < DATA_SIZE; i = i + 1) begin
          for (j = 0; j < NUM_COMP; j = j + 1) begin
            memory[i][MEMORY_WIDTH*j +: MEMORY_WIDTH] = mem_data[i*NUM_COMP+j][MEMORY_WIDTH*comp_no +: MEMORY_WIDTH];
          end
        end
        $sformat(mem_read_file, "mem_init_%0d.hex", comp_no);
        $writememh(mem_read_file, memory);

        memory_used = DATA_SIZE;
        count_read = 0;
        count_write = 0;
        for (memory_index=0; memory_index<memory_used; memory_index=memory_index+1) begin
          address[memory_index] = memory_index;
        end
        check_init = 1;
      end
    end
    ...
    task data_task;
        ...
        begin
            ...
            if (dq_in_valid) begin // write data
                ...
                if (DEBUG) $display ("%m: at time %t INFO: WRITE @ DQS= bank = %h row = %h col = %h data = %h burst_cntr= = %d",$time, bank, row, (-1*BL_MAX & col) + burst_position, dq_temp, burst_cntr);
